"""
绘制训练loss曲线并检测过拟合
支持从 checkpoint 文件或 JSON 文件读取训练历史
"""
import json
import os
import glob
...
def load_history_from_checkpoints(checkpoint_dir='checkpoints'):
    ...
    每个 checkpoint 文件包含: epoch, train_loss, val_loss
    """
    # 查找所有 checkpoint 文件（包括 best_model.pth）
    checkpoint_files = glob.glob(os.path.join(checkpoint_dir, 'checkpoint_epoch_*.pth'))
    best_model_path = os.path.join(checkpoint_dir, 'best_model.pth')
    
    # 如果存在 best_model.pth，也加入列表
...
def main():
    ...
    parser.add_argument('--history_path', type=str, 
                       default=None,
                       help='训练历史JSON文件路径（如果提供，则从JSON读取；否则从checkpoint文件读取）')
    ...
    try:
        if args.history_path and os.path.exists(args.history_path):
            ...
            print(f"成功从 JSON 文件加载训练历史: {args.history_path}")
        else:
            # 从 checkpoint 文件读取
            history = load_history_from_checkpoints(args.checkpoint_dir)
            print(f"成功从 checkpoint 文件加载训练历史: {args.checkpoint_dir}")
        
        print(f"总训练轮数: {len(history['epoch'])}")
        print(f"Epoch 范围: {min(history['epoch'])} - {max(history['epoch'])}")
    except (FileNotFoundError, ValueError) as e:
        print(f"错误: {e}")
        print("\n请确保:")
        print("  1. checkpoint 文件存在于指定目录")
        print("  2. checkpoint 文件中包含 epoch, train_loss, val_loss 信息")
        return
