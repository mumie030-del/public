    ...
    with torch.no_grad():
        for i, (images, masks) in enumerate(tqdm(test_loader, desc="Testing")):
            ...
            if num_features >= 2:
                ...
                # 叠加半透明颜色：左肾绿色，右肾红色
                overlay[:, :, 1] = np.clip(overlay[:, :, 1] + left_kidney * 0.6, 0, 1)   # 绿色
                overlay[:, :, 0] = np.clip(overlay[:, :, 0] + right_kidney * 0.6, 0, 1)  # 红色
                ...
                # 只有一个连通域，标成黄色
                overlay[:, :, 0] = np.clip(overlay[:, :, 0] + pred_show * 0.6, 0, 1)
                overlay[:, :, 1] = np.clip(overlay[:, :, 1] + pred_show * 0.6, 0, 1)
