...
# 2. 保留你的 Transformer 模块
class TemporalTransformer(nn.Module):
    def __init__(self, input_dim=1, num_heads=8, num_layers=4, dropout=0.1):
        super(TemporalTransformer, self).__init__()
        # 注意：这里 batch_first=True，意味着输入需要是 (Batch, Seq_Len, Feature_Dim)
...
class Unet(nn.Module):
    def __init__(self, in_channels=26, out_channels=1):
        ...
        # 【新增】：在这里插入 Transformer
        # 瓶颈层的输出通道数是 1024，所以 Transformer 的输入维度也是 1024
        self.transformer = TemporalTransformer(input_dim=1024, num_heads=8, num_layers=4)
    ...
    def forward(self, x):
        ...
        # 2. 瓶颈层
        bottleneck = self.bottleneck(p4) # 此时形状: (Batch, 1024, 16, 16)
        
        # 【新增】：维度魔法与 Transformer 交互
        b, c, h, w = bottleneck.shape
        # 展平空间维度并交换：(Batch, 1024, 16, 16) -> (Batch, 1024, 256) -> (Batch, 256, 1024)
        x_seq = bottleneck.view(b, c, h * w).permute(0, 2, 1)
        
        # 过 Transformer 提取全局依赖
        x_trans = self.transformer(x_seq)
        
        # 恢复回原来的图像维度：(Batch, 256, 1024) -> (Batch, 1024, 256) -> (Batch, 1024, 16, 16)
        bottleneck = x_trans.permute(0, 2, 1).view(b, c, h, w).contiguous()
